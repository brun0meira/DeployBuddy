    ...
    <Container>
      ...
      <ContainerContent>
        ...
        <Header>
            <h1>PagSeguro</h1>
            <Navigation>
                <TextHeader data-testid="management-view" selected={viewMode === 'management'} onClick={() => handleViewSelect('management')}>Gerenciamento</TextHeader>
                <span>|</span>
                <TextHeader data-testid="history-view" selected={viewMode === 'history'} onClick={() => handleViewSelect('history')}>Histórico</TextHeader>
            </Navigation>
        </Header>
        <DragDropContext onDragEnd={handleDragEnd}>
          <KanbanContainer>
            {viewMode === 'management' ? (
              <KanbanBase data-testid="kanban-base">
                {project.ambientes.map((ambiente, index) => (
                  ambiente.nome === 'Repositório Local' ? (
                    <KanbanColumn key={ambiente.nome} nome={ambiente.nome} cards={ambiente.cards as unknown as Card[]} />
                  ) : (
                    <KanbanColumn key={ambiente.nome} nome={ambiente.nome} cards={ambiente.cards as unknown as Card[]} />
                  )
                ))}
              ...
              <HistoryBase data-testid="history-base">Histórico</HistoryBase>
            )}
          </KanbanContainer>
